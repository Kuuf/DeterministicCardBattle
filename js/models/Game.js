class Game {
    ...
    findTargetOptimizeDamage(attacker, targets) {
        ...
        for (let target of targets) {
            const remainingDamage = (target.currentHp - attacker.attack)
    ...
    findTargetKillShot(attacker, targets) {
        ...
        const killableTargets = targets.filter(t => attacker.attack >= t.currentHp);
    ...
    async showDamageNumber(targetElement, damage) {
        ...
        damageNum.style.left = `${rect.left + rect.width / 2 - boardRect.left}px`;
        damageNum.style.top = `${rect.top - boardRect.top}px`;
        
        gameBoard.appendChild(damageNum);
        
        this.setTimeout(() => damageNum.remove(), 1000);
    }
    ...
    async showManaCost(playerPrefix, manaCost) {
        ...
        const manaNum = document.createElement('div');
        manaNum.className = 'mana-cost';
        manaNum.textContent = `-${manaCost}`;
        manaNum.style.left = `${rect.left + rect.width / 2 - boardRect.left}px`;
        manaNum.style.top = `${rect.top - boardRect.top}px`;
        
        gameBoard.appendChild(manaNum);
        
        this.setTimeout(() => manaNum.remove(), 1000);
    }
    ...
    async showManaGain(playerPrefix, manaGain) {
        ...
        const manaNum = document.createElement('div');
        manaNum.className = 'mana-gain';
        manaNum.textContent = `+${manaGain}`;
        manaNum.style.left = `${rect.left + rect.width / 2 - boardRect.left}px`;
        manaNum.style.top = `${rect.top - boardRect.top}px`;
    ...
    async showDirectDamage(playerName, damage) {
        ...
        damageDisplay.className = 'direct-damage';
        damageDisplay.textContent = `-${damage} HP`;
        damageDisplay.style.left = `${rect.left + rect.width / 2 - boardRect.left}px`;
        damageDisplay.style.top = `${rect.top - boardRect.top}px`;
        
        gameBoard.appendChild(damageDisplay);
        ...
        for (let {card, attacker, defender} of queue) {
            ...
            if (target) {
                ...
                if (targetElement) {
                    ...
                    this.log(`${attacker.name}'s ${card.name} attacks ${defender.name}'s ${target.name} for ${card.attack} damage!`);
                    
                    await this.sleep(600);
                    
                    const died = target.takeDamage(card.attack);
                    ...
                    await this.showDamageNumber(targetElement, card.attack);
                ...
                this.log(`💥 ${attacker.name}'s ${card.name} attacks ${defender.name} directly for ${card.attack} damage!`);
                
                await this.createAttackArrow(attackerElement, null, defender, card);
                await this.sleep(600);
                
                defender.hp -= card.attack;
                await this.showDirectDamage(defender.name, card.attack);
    ...
    createCardElement(card, isNewCard = false, shouldSlide = false) {
        ...
        const attackBoosted = card.deckType === 'angry' ? 'boosted' : '';
